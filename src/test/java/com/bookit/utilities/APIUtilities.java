...
public class APIUtilities {
    ...
    /**
     * This methods is used to retrieve token from the server
    ...
    /**
     * This methods is used to retrieve authorization token from the server for specific role.
     * Token must be attached to the header of every API call
     * @return token
     */
     ...
     * @param email
     * @param password
     * @return user id return -1 if user does not exist
     */
    public static int getUserID(String email, String password){
        try {
            Response response = given().auth().oauth2(getToken(email, password)).when().get(Endpoints.GET_ME);
            response.then().log().ifError();// prints response details in case of error
            response.then().statusCode(200);// ensure that it returns 200
    ...
    public static Response deleteUserByID(int id){
        ...
        response.then().statusCode(204).log().ifError();
    ...
    public static Response addBatch(int batchNumber){
        ...
        Response response = given().auth().oauth2(token).queryParam("batch-number",batchNumber).post(Endpoints.ADD_BATCH);
     ...
     * If user exists - this method will delete that user
     * @param email
     * @param password
     */
